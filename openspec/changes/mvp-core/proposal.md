...
- **Images / photos / screenshots** — multimodal LLM (or vision) extracts candidate words/phrases from the image.
...
- Python + aiogram 3.x + OpenAI (model with vision capability, cost-efficient tier where possible).

**Out of scope for MVP (Phase 2+):**
- Voice messages / STT (audio recognition deferred).
...
- A new user can start a conversation, send a word **or an image**, confirm addition, and later receive / request a review of that item.
- All bot replies stay short and actionable.
- Engineers can implement and demo the full loop end-to-end (text + image paths).
...
| LLM                | OpenAI with vision support (cost-efficient where possible) |
